...
class AutoVacuum(hass.Hass, mqtt.Mqtt):
    ...
    def initialize(self):
        ...
        for item, config in self.args["zones"].items():
            ...
            self.set_state(f'sensor.{self.mqtt_topic_prefix}_{zone["vacuum"]}_opvangbak', state=zone["area_cleaned"])
            ...
            self.listen_event(self.update_cleaned_area, "MQTT_MESSAGE", namespace="mqtt", topic=f'{self.mqtt_topic_prefix}/{zone["vacuum"]}/CurrentStatisticsCapability/area', zone=zone)
